...
// Initialize GA4
export const GA_TRACKING_ID = process.env.NEXT_PUBLIC_GA_TRACKING_ID || 'G-FVQVG1PN8K';

// Debug logging
if (typeof window !== 'undefined') {
  console.log('🔍 GA4 Debug Info:', {
    trackingId: GA_TRACKING_ID,
    envVar: process.env.NEXT_PUBLIC_GA_TRACKING_ID,
...
export const trackSignup = (caseId) => {
  console.log('🎯 Tracking signup event:', { caseId, trackingId: GA_TRACKING_ID });
  ...
  if (typeof window !== 'undefined' && window.gtag) {
    console.log('📊 Sending GA4 signup event');
    window.gtag('event', 'signup', {
      case_id: caseId,
      event_category: 'conversion'
    });
  } else {
    console.warn('⚠️ gtag not available for signup tracking');
  }
};
