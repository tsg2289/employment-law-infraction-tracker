...
export default async function handler(req, res) {
  ...
  try {
    // Insert leave of absence violation data with user_id for RLS
    const { data, error } = await supabase
      .from('leave_of_absence_violations')
      .insert({
        user_id: user.id,
        case_id: caseId,
        name: trim(name),
    ...
    if (error) {
      ...
      return res.status(500).json({ error: 'Failed to save leave of absence info' })
    }
